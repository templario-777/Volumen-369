...
# FUNCIONES TÉCNICAS
# =========================
def obtener_ohlcv(exchange, symbol, timeframe='5m', limit=1000):
    try:
...
def calcular_indicadores(df):
    ...
    df['extra_vol'] = df['volume'] > df['volume'].rolling(1000).mean()
    return df.dropna()
...
def detectar_patron(df):
    ...
    # Check for Demand Zone (Bullish)
    if (row['bull_candle'] and prev1['bull_candle'] and prev2['bull_candle'] and 
        prev1['extra_vol']):
        for i in range(0, 6):
            if len(df) > i + 1 and df.iloc[-i - 1]['bear_candle']:
                demand_zone = True
                fuerza = min(100, int(prev1['rvol'] * 40))
                break
    
    # Check for Supply Zone (Bearish)
    if (row['bear_candle'] and prev1['bear_candle'] and prev2['bear_candle'] and 
        prev1['extra_vol']):
        for i in range(0, 6):
            if len(df) > i + 1 and df.iloc[-i - 1]['bull_candle']:
                supply_zone = True
                fuerza = min(100, int(prev1['rvol'] * 40))
                break
    
    # Original signal logic as backup
    alcista = row['sma50'] > row['sma200']
    bajista = row['sma50'] < row['sma200']
    ruptura_alcista = row['close'] > prev1['high'] and row['volume'] > prev1['volume'] * 1.5
    ruptura_bajista = row['close'] < prev1['low'] and row['volume'] > prev1['volume'] * 1.5
    vol_explosivo = row['rvol'] > 2.0
    
    if demand_zone or (alcista and ruptura_alcista and vol_explosivo):
        return "BUY", max(fuerza, min(100, int(row['rvol'] * 30)))
    if supply_zone or (bajista and ruptura_bajista and vol_explosivo):
        return "SELL", max(fuerza, min(100, int(row['rvol'] * 30)))
...
def main():
    ...
    try:
        ...
        # Selector de monedas
        st.sidebar.subheader("📊 Seleccionar Criptomonedas")
        selected = st.sidebar.multiselect(
            "Elige las monedas a monitorear",
            options=sorted(markets),
        ...
        if st.sidebar.button("🔍 Seleccionar Top 10", type="primary"):
            with st.spinner("Analizando mercado..."):
                st.session_state.selected_symbols = markets[:TOP_N]
    ...
    if st.session_state.selected_symbols:
        st.subheader(f"📋 Monitoreo 24/7 - {len(st.session_state.selected_symbols)} monedas")
        ...
        for symbol in st.session_state.selected_symbols:
            try:
                ...
                # Agregar gráfico de TradingView
